...
package body Gnoga.Server.Connection is
   ...
   begin
      ...
      begin
         ...
         if not Secure_Server then
            Standard.Ada_Lib.Trace.Log_Here (Debug, "port" & Server_Port'img);
            declare
   ...
   begin
      return Gnoga.Server.Connection.Common.Gnoga_Client_Factory
        (Listener       => Connections_Server_Ptr (Listener), --.all'Unchecked_Access,
   ...
   protected body Connection_Manager is
      ...
      begin
         Standard.Ada_Lib.Trace.Log_In (Debug, "ID" & ID'img);
         if ID > 0 then
            ...
            if Socket_Map.Contains (ID) then
               if    Current_Socket /= Socket_Maps.No_Element and then
                     Socket_Maps.Key (Current_Socket) = ID then
                  Standard.Ada_Lib.Trace.Log_Here (Debug);
...
--Standard.Ada_Lib.Trace.log_here;
   ...
   begin
      ...
      if F /= "gnoga" and Index (F, "gnoga?") = 0 then
         ...
         begin
            Standard.Ada_Lib.Trace.Log_Out (Debug, "reason " & Reason);

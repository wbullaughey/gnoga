...
package body GNAT.Sockets.Server is
   ...
   begin
      ...
      if Client.Connect_No > Client.Max_Connect_No then
         ...
         Clear (Client.all);
         pragma assert (false, "need to fix code below");
         Connect_Socket
         (  Socket   => Client.Socket,
            Server   => Client.Client_Address,
            Timeout  => 0.0,
            Selector => null,    -- could not get to compileGNAT.Sockets.Selector_Access'(Listener.Selector'Unchecked_Access),
            Status   => Status
         );
...
ada_lib.Trace.log_out (true);
   ...
   procedure Stop
      ...
      Reconnect  : Boolean := not Listener.Finalizing
                         and then Client.Session /= Session_Down
                         and then not Client.Shutdown_Request;
   begin
      Trace_Sending
      (  Listener.Factory.all,
         Client.all,
